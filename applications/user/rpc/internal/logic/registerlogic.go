...
func (l *RegisterLogic) Register(in *service.RegisterRequest) (*service.RegisterResponse, error) {
	...
	if err != nil {
		...
		return nil, xcode.Errorf(xcode.DatabaseError, "注册失败")
	}
...
func (l *RegisterLogic) checkUserExist(username, phone string) error {
	...
	if err != nil {
		...
		return xcode.Errorf(xcode.DatabaseError, "查询用户失败")
	}
	if user != nil {
		logx.Infof("Register req: %v error: %v", username, err)
		return xcode.Infof(xcode.UserHasRegistered, "用户名已经注册")
	}
	...
	if user != nil {
		logx.Infof("Register req: %v error: %v", phone, err)
		return xcode.Infof(xcode.UserHasRegistered, "手机号已经注册")
	}
	return nil
}
