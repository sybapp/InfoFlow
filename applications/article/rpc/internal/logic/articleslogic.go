...
import (
	...
	"github.com/sybapp/infoflow/applications/article/rpc/internal/code"
	...
	"github.com/sybapp/infoflow/applications/article/rpc/internal/types"
...
func (l *ArticlesLogic) Articles(in *pb.ArticlesRequest) (*pb.ArticlesResponse, error) {
	...
	if len(articleIds) > 0 {
		...
		v, err, _ := l.svcCtx.SingleFlightGroup.Do(fmt.Sprintf("ArticlesByUserId:%d:%d", in.UserId, in.SortType), func() (interface{}, error) {
			return l.svcCtx.ArticleModel.FindArticlesByUserId(l.ctx, in.UserId, sortLikeNum, sortPublishTime, sortField, types.DefaultLimit)
		})
		if err != nil {
